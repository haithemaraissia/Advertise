...
public partial class Advertiser_Compaign_Completed_ChangeAd : System.Web.UI.Page
{
    bool _flag = true;
    ...
    protected void UpdateAdButtonClick(object sender, EventArgs e)
    {
            ...
            if (Page.Validators[0].IsValid && Page.Validators[1].IsValid && Page.Validators[2].IsValid && Page.Validators[4].IsValid)
            {
                if (AsyncFileUpload1.HasFile)
                {
                    if (Page.Validators[3].IsValid)
                    {
                        ...
                        UpdateAd(adPath, 0);
                        SaveCreative();
                        Response.Redirect(string.Format("ReviewAd.aspx?ID={0}&P=Completed", GetID()));
                    }
                }
                else
                {
        ...
        catch (Exception)
        {
            ...
            Response.Redirect("Completed/Inventory.aspx");
        }
    }
