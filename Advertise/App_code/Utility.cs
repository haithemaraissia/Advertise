...
namespace Admanagment.Helpers
{
    public static class Utility
    {
        ...
        public static string GetRegion(DropDownList region)
        {
            switch (region.SelectedIndex)
            {
                case -1:
                    return region.Items[0].Text;
                default:
                    return region.SelectedValue;
            }
        }
    ...
    return selectedProfession;
        ...
        public static string GetProfessionID(ListBox specialityListBox1, ListBox specialityListBox2, ListBox specialityListBox3)
        {
            ...
            {
                selectedProfessionID = selectedProfessionID.Remove(selectedProfessionID.Length - 1);
            }
            return selectedProfessionID;
        }

        public static string CleanUpProfessionId(string professionId)
        {
            if (professionId.Substring(professionId.Length-1, 1)== ",")
            {
                professionId = professionId.Remove(professionId.Length - 1);
            }
            return professionId;
        }
        ...
        public static string GetSideJobSiteID()
        {
            return "1";
            /*Documentation:
       ...
       // }
        ...
        public static string KeywordFiltering(string positionid, string sectionId, string querystring)
        {
            var siteId = GetSideJobSiteID();
            var language = GetCurrentLCID(querystring);
            var sample = siteId + "," + sectionId + "," + positionid + "," + language + ",";
            Logic.GetKeywordAttributes(Convert.ToInt32(sectionId));

            if (Logic.Gender == null && Logic.Age == null && Logic.Country == null && Logic.Region == null)
            {
                if (positionid == "1") return Logic.DefaultTopNoAuthenticatedROSKeyword;
                if (positionid == "1") return Logic.DefaultRightNoAuthenticatedROSKeyword;
            }
            sample += Logic.Gender + "," + Logic.Age + "," + Logic.Country + "," + Logic.Region + "," + Logic.City +
                      "," + Logic.Zipcode + ",";
            sample += Logic.Industry + "," + Logic.NumberofProfession + "," + Logic.Profession;
            return sample;
        }
        ...
        public static string GetAdvertiserLocation(string city, string region, string country)
        {
            var location = "";
            if (city != "null" && city != "-1")
            {
        ...
        static Dictionary<string, int> _indsutryDicionary = new Dictionary<string, int>()
	    {
            ...
            {"Professional and Technical Service", 13},
	        {"Real Estate and Utilities", 14},
	        {"Trade and Management of Enterprises", 15},
	        {"Transportation and Warehousing", 16},
            {"Environment and Aerospace", 17},
	        {"Media and Telecommunication", 18},
	        {"Public and Private Services ", 19},
	    };
