    ...
    public partial class ChangeAd : System.Web.UI.Page
    {
        ...
        protected void UpdateAdButtonClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Page.Validators[0].IsValid && Page.Validators[1].IsValid && Page.Validators[2].IsValid && Page.Validators[4].IsValid)
                {
                    if (AsyncFileUpload1.HasFile)
                    {
                        ...
                        {
                            var adPath = "../Advertiser/Ads/" + User.Identity.Name + "/" + AsyncFileUpload1.FileName.ToString(CultureInfo.InvariantCulture);
                            UpdateAd(adPath, 1);
                            SaveCreative();
                            Response.Redirect((string.Format("ReviewAd.aspx?ID={0}&P=Temp", GetAdID())),false);
                        return;
                        }
                    }
                    else
            ...
            catch (Exception e1)
            {
                ...
                Response.Redirect("~/Advertiser/Compaign/Current/Inventory.aspx");
            }
        ...
        protected void DeleteAddButton_Click(object sender, EventArgs e)
        {
            try
            {
                Helpers.Utility.DeleteAd(GetAdID());
