...
{
    public partial class AdvertiserReportCompletedProfile : Page
    {
        ...
        protected void ValidateAdvertiser()
        {
            ...
            if (user == null)
            {
                Response.Redirect("../../../Account/Login.aspx?ReturnUrl=../Advertiser/Report/Current/Profile.aspx");
            }
        }
        ...
        protected void FilterButtonClick(object sender, EventArgs e)
        {
            ...
            dsdetails.SelectCommand =
                "SELECT DISTINCT TOP (10000) AdGeneral.AdName, AdStats.EntryDate, AdGeneral.ImageUrl, VisitorInfo.AdId, VisitorInfo.UserId," +
                " VisitorInfo.UserRole,     VisitorInfo.FirstName, VisitorInfo.LastName, VisitorInfo.Address, VisitorInfo.CountryName, VisitorInfo.RegionName," +
                " VisitorInfo.CityName, VisitorInfo.Age, VisitorInfo.Gender, VisitorInfo.PhotoPath, VisitorInfo.IndustryID, VisitorInfo.ProfessionID FROM AdGeneral" +
                " INNER JOIN AdStats ON AdGeneral.AdID = AdStats.AdId INNER JOIN VisitorInfo ON AdGeneral.AdID = VisitorInfo.AdId  " +
        ...
        protected void PDFButtonClick(object sender, ImageClickEventArgs e)
        {
            ...
            Response.Redirect("IFrame/ProfileDownload.aspx?t1=" + t + "&t2=" + t2);
        }
        ...
        protected void ViewAllAdImageButtonClick(object sender, ImageClickEventArgs e)
        {
            dsdetails.SelectCommand =
                ...
                "INNER JOIN VisitorInfo ON AdGeneral.AdID = VisitorInfo.AdId  WHERE AdGeneral.UserID = '" + Utility.GetUserID() + "' AND AdGeneral.IsActive = 0 AND " +
                "AdGeneral.Completed = 1 ORDER BY AdStats.EntryDate";
            gvdetails.DataBind();
