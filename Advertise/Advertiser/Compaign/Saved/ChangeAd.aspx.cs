...
public partial class Advertiser_Compaign_Saved_ChangeAd : System.Web.UI.Page
{
    bool _flag = true;
    ...
    protected void GetAdProperties()
    {
        ...
        using (var context1 = new AdDatabaseModel.AdDatabaseEntities())
        {
            ...
            PositionDropDownList.SelectedIndex = Convert.ToInt32(adgeneral[0].PositionID);
        ...
        try
        {
            ...
            if (Page.Validators[0].IsValid && Page.Validators[1].IsValid && Page.Validators[2].IsValid && Page.Validators[4].IsValid)
            {
                if (AsyncFileUpload1.HasFile)
                {
                    ...
                    {
                        var adPath = "http://www.haithem-araissia.com/WIP2/RightCleanSideJOB2008FromInetpub/WorkingCopy/Web/Advertise/Advertiser/Ads/" + User.Identity.Name + "/" + AsyncFileUpload1.FileName.ToString(CultureInfo.InvariantCulture);
                        UpdateAd(adPath, 0);
                        SaveCreative();
                        Response.Redirect((string.Format("ReviewAd.aspx?ID={0}&P=Saved", GetID())),false);
                        return;
                    }
                }
                else
    ...
    protected void DeleteAddButton_Click(object sender, EventArgs e)
    {
        try
        {
            Admanagment.Helpers.Utility.DeleteAd(GetAdID());
            Response.Redirect("Inventory.aspx");
        }
        catch (Exception)
        {
