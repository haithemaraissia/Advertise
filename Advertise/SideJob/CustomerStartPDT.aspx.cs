...
public partial class CustomerStartPDT : System.Web.UI.Page
{
    ...
    protected double GetPaymentDue()
    {
        ...
        int adID = GetAdID();
        var payment = (from payments in context.AdvertiserPaymentDues
                       where payments.PaymentDueID == paymentdue && payments.AdID == adID
                       select payments).FirstOrDefault();
    ...
    protected bool CheckPreviousPayment()
    {
        ...
        var archivedpaymentid = (from archivedpayment in context.ArchivedAdvertiserPayments
                                 where archivedpayment.PaymentDueID == payment && archivedpayment.AdID == adID
    ...
    {
        return int.Parse(Request.QueryString["AD"]);
    }
}