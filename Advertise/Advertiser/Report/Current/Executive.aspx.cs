...
{
    public partial class Executive : Page
    {
        ...
        protected void FilterButtonClick()
        {
            ...
            dsdetails.SelectCommand =
                          ...
                          + t2 + "', AdGeneral.EndDate) <= 0) AND AdGeneral.UserID = '" + Utility.GetUserID() + "' And (AdGeneral.IsActive = 1)";
            gvdetails.DataBind();
            ...
            using (var context = new AdDatabaseModel.AdDatabaseEntities())
            {
                ...
                if (!(t1 == null || t2 == null))
                {
                    y = (from g in context.AdGenerals
                         join s in context.AdTotalStats
                             on g.AdID equals s.AdId
                         where g.StartDate >= t1 && g.EndDate <= t2 && g.UserID == id && g.IsActive
                         select new ExecutiveReport
                         {
                             ID = g.AdID,
                             ImageUrl = g.ImageUrl,
